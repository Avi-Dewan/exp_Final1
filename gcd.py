...
def CE_PI_CL_softBCE_train(model, 
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, ((x, x_bar), label, idx) in enumerate(tqdm(unlabeled_train_loader)):
            ...
            pairwise_pseudo_label[rank_diff>0] = -1 

            prob_pair, _ = PairEnum(prob)
            _, prob_bar_pair = PairEnum(prob_bar)

            bce_loss = criterion_hard_bce(prob_pair, prob_bar_pair, pairwise_pseudo_label)
...
if __name__ == "__main__":
    ...
    model = ResNet(BasicBlock, [2,2,2,2], args.n_labeled_classes).to(device)
